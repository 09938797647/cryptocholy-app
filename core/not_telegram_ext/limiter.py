...
def deduplicate_response(method: Callable):
    @wraps(method)
    async def inner(
        self: BaseRateLimiter,
        callback: Callable[
            ..., Coroutine[Any, Any, Union[bool, JSONDict, List[JSONDict]]]
        ],
        args: Any,
        kwargs: Dict[str, Any],
        endpoint: str,
        ...
        _command, _kwargs = args
        _key, _value = format_response_cache_key_value(_command, _kwargs)
        if _key:
            if check_user_action_cache(_key, _value):
                _LOGGER.debug("Duplicated requests for chat ID %s. Exit", _key)
                return None

            add_user_action_cache(_key, _value)
...
class NotAIORateLimiter(BaseRateLimiter[int]):
    """
    ...
    def __init__(
        ...
        write_max_rate: float = 30,
        write_time_period: float = 1,
        ...
        if write_max_rate and write_time_period:
            self._write_limiter: Optional[AsyncLimiter] = AsyncLimiter(
                max_rate=write_max_rate,
                time_period=write_time_period,
            )
    ...
    async def _run_request(
        ...
        base_context = (
            self._base_limiter if (chat and self._base_limiter) else nullcontext()
        )
        write_context = (
            self._write_limiter if (chat and self._write_limiter) else nullcontext()
        )
        group_context = (
        ...
        if isinstance(args[0], str) and args[0].startswith("send"):
            async with group_context, base_context, write_context:
                _LOGGER.debug("Processing %s with write limiter", args[0])
                # In case a retry_after was hit, we wait with processing the request
                await self._retry_after_event.wait()

                return await callback(*args, **kwargs)

        elif args[0] == "answerCallbackQuery":
            _LOGGER.debug("Processing %s without limiter", args[0])
            # In case a retry_after was hit, we wait with processing the request
            await self._retry_after_event.wait()
            ...
            async with group_context, base_context:
                _LOGGER.debug("Processing %s with default limiter", args[0])
                # In case a retry_after was hit, we wait with processing the request
                await self._retry_after_event.wait()
    ...
    async def process_request(
        ...
        data: Dict[str, Any],
        rate_limit_args: Optional[int],
    ) -> bool | JSONDict | List[JSONDict] | None:
        """
